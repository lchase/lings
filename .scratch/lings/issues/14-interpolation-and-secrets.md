...
**Blocked by:** 13 — Bash step execution.

**Status:** ready-for-agent

- [ ] `secrets` table: `folder_id` (nullable = global), `name`, `encrypted_value`, encrypted at rest with a server-held key.
- [ ] Secret resolution uses the same nearest-ancestor-then-global-fallback rule as laws docs (ticket 05).
