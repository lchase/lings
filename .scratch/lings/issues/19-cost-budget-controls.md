...
**Blocked by:** 16 — agent-call step + approval gate, 09 — Bots: entity + ad-hoc chat.
...
- [ ] A session's live `cost` events (from ticket 07/08) are checked against its cap; crossing it hard-stops the session (or halts it to `waiting-approval` — pick one and apply consistently).
