...
**Blocked by:** 12 — Playbooks: CRUD/DAG editor, lock/versioning.

**Status:** ready-for-agent

- [ ] `playbook_runs` table: `playbook_id` (nullable), `version_id` FK pinning to the version current at spawn time, `ticket_id` (nullable, unused until ticket 16).
...
- [ ] An `approval` step blocks the run until resolved (resolution can be a bare "approve" action for this ticket — full session `waiting-approval` wiring lands in ticket 15).
